...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, return_attention=False, return_attention_logits=False):
        B, N, C = x.shape
        ...
        # Fast SDPA path unless attention probabilities/logits are explicitly requested.
        if not return_attention and not return_attention_logits:
            ...
            # Measurement path.  Keep the PRE-softmax logits so canonical ADS can
            # be evaluated with float64 log-softmax arithmetic without a floor.
        ...
        if return_attention_logits:
            return x, attn_weights, attn_logits
        if return_attention:
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, return_attention=False, return_attention_logits=False):
        if return_attention_logits:
            attn_out, attn_weights, attn_logits = self.attn(
                self.norm1(x), return_attention=True, return_attention_logits=True)
            x = x + attn_out
...
class VisionTransformer(nn.Module):
    ...
    def forward_with_attention_logits(self, x):
        """Measurement forward returning native attention probabilities and pre-softmax logits ON DEVICE.

        v1.4.1 deliberately keeps both tensors on the accelerator.  The v1.4
        probe moved every layer to CPU and then evaluated float64 log-softmax/KL
        there, which made the numerical lock probe CPU-bound on FMLE.

        Model behavior is unchanged: the value path still uses the native-dtype
        softmax weights.  Returned tensors are detached but remain on the same
        device as ``x`` so the caller can perform float64 divergence arithmetic
        on the H200 and transfer only final per-image scalars to CPU.
        """
        B = x.shape[0]
        x = self.patch_embed(x)
        cls = self.cls_token.expand(B, -1, -1)
        ...
        attention_logits = []
        for block in self.blocks:
            x, attn, logits = block(x, return_attention_logits=True)
            attentions.append(attn.detach())
            attention_logits.append(logits.detach())

        x = self.norm(x)
        return self.head(x[:, 0]), attentions, attention_logits
