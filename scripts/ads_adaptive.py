#!/usr/bin/env python3
"""
ads_adaptive.py — Adaptive ADS attacker experiment (ImageNet-100, n=6).

This patched version fixes the adaptive-attack objective and uses the full
256-image ADS reference set for the attack objective by default.

Key conventions
---------------
* PGD is implemented as projected *ascent* on

      J_adaptive = CE(logits_ref, y_ref) - lambda * ADS_L4(delta; D_ref)

  so lambda=0 is standard CE-ascent PGD, and lambda>0 trades accuracy damage
  against ADS minimization.
...
* ADS(L4) is reported on the full 256-image reference set.
...
# -----------------------------------------------------------------------------
# Defaults from the final ImageNet-100 n=6 protocol
# -----------------------------------------------------------------------------
DEFAULT_PE_TYPES = ["learned", "rope"]
DEFAULT_SEEDS = [42, 123, 456, 789, 1011, 1213]
DEFAULT_EPSILONS = [0.05, 0.1, 0.2]
DEFAULT_LAMBDAS = [0.0, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 50.0]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Adaptive ADS attacker experiment (ImageNet-100, n=6).",
    ...
    p.add_argument("--lambdas", nargs="+", type=float, default=None,
                   help="Adaptive regularization strengths")
...
def get_attn_l4(model: nn.Module, loader: DataLoader) -> torch.Tensor:
    """Exact image-averaged Layer-4 attention tensor, shape (H, N, N)."""
    total_attn = None
    total_images = 0
...
@torch.no_grad()
def compute_ads_l4_scalar(clean_attn: torch.Tensor, perturbed_attn: torch.Tensor) -> float:
    eps = 1e-10
...
def compute_full_ref_objective(
    ...
    if lam > 0:
        ...
        q = q / q.sum(-1, keepdim=True)
        p = clean_attn.detach().to(obj_device).float() + eps_kl
        p = p / p.sum(-1, keepdim=True)
...
def adaptive_pgd(model: nn.Module, clean_attn: torch.Tensor, pe_type: str, epsilon: float, lam: float) -> nn.Module:
    ...
    for _ in range(PGD_STEPS):
        ...
        _, _, objective = compute_full_ref_objective(pm, clean_attn, criterion, lam)
        objective.backward()
        ...
        for seed in SEEDS:
            ...
            for eps in EPSILONS:
                ...
                for lam in LAMBDAS:
                    ...
                    evades = ads_ratio < tau
                    print(
                        f" λ={lam:5.1f}: acc_drop={acc_drop:6.2f}pp, "
...
def analyze(results: dict) -> None:
    ...
    for pe_type in PE_TYPES:
        ...
        for eps in EPSILONS:
            eps_key = str(eps)
            print(f"\n ε={eps}:")
            print(f" {'λ':>7} {'drop mean':>10} {'drop sd':>9} {'ADS ratio':>10} {'evades':>10}")
            print(" " + "-" * 54)
            for lam in LAMBDAS:
                lam_key = str(lam)
                drops, ratios, evades = [], [], []
                ...
                print(
                    ...
                    f"{np.mean(ratios):10.2f} {sum(evades)}/{len(evades):<6}"
                )


if __name__ == "__main__":
    print("Adaptive Attacker Experiment — patched full-reference CE - lambda*ADS")
