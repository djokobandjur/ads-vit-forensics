...
"""
Layer-wise Positional Probing — Residual Stream (CIFAR-100, n=6)
================================================================
...
4 PE types × 6 seeds = 24 CIFAR-100 ViT-Base checkpoints.
...
Protocol:
  ...
  - CIFAR-100 geometry: 32×32 images, 4×4 patches,
    8×8 grid = 64 patch tokens.
  - Feature: residual-stream vector at layer l, shape (768,).
  - Target: patch row ∈ [0,7] and col ∈ [0,7].
  - Probe: Ridge regression with image-level GroupKFold CV (5 folds).
  - Metric: R²(row), R²(col), R²(mean).
  - Reference set: 256 images × 64 patches = 16,384 samples per layer.
...
Output:
  ...
  - ads_ref_indices_cifar100.json if no reference index file is supplied
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Residual-stream positional probing (CIFAR-100, n=6). See module docstring for details.",
