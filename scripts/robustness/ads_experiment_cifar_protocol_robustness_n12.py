...
Default n=12 canonical protocol-robustness protocol:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ADS main experiment (CIFAR-100). See module docstring for details.",
    ...
    parser.add_argument(
        ...
        help='Optional. Path to torchvision CIFAR-100 cache directory (default: /tmp/cifar100). '
             'CIFAR-100 is auto-downloaded by torchvision if not present.',
    )
    parser.add_argument(
        '--output_path',
        type=str,
...
def perturb_pe_alibi(model, epsilon):
    ...
    # Convert slopes buffers to Parameters in all ALiBi / 2D-ALiBi modules
    alibi_params = []
    alibi_params.extend(convert_any_slope_buffers_to_params(perturbed_model))
...
def perturb_pe_pgd(model, pe_type, epsilon):
    ...
    for name, param in list(perturbed_model.named_parameters()):
        ...
        if any(k in clean for k in ['pos_embed', 'slope', 'inv_freq']):
        ...
        for seed in SEEDS:
            ...
            for eps in EPSILONS:
                ...
                if eps == 0.0:
                    ...
                    perturbed_model = perturb_pe_pgd(model, get_model_pe_type(pe_type), eps)
...
def plot_ads_results(results):
    ...
    # Unified figure typography for paper-ready robustness figures.
    PLOT_FONT_SIZE = 13
    plt.rcParams.update({
    ...
    def format_condition_label(pe_type):
        return {
            'alibi': '1D-ALiBi',
            'alibi_2d': '2D-ALiBi',
        ...
        }.get(pe_type, pe_type.replace('_', ' ').title())
    ...
    fig.suptitle('Attention Divergence Score (L4) vs ε', fontsize=PLOT_FONT_SIZE, fontweight='bold')
    for idx, pe in enumerate(PE_TYPES):
        ...
        ads_vals = np.array([results[pe][s]['ads_layer4'] for s in available_seeds])
        mean_ads = ads_vals.mean(0)
        std_ads  = ads_vals.std(0, ddof=1) if len(available_seeds) > 1 else np.zeros_like(mean_ads)
        ...
        ax.set_title(format_condition_label(pe), fontsize=PLOT_FONT_SIZE, fontweight='bold', color=PE_COLORS[pe])
        ax.tick_params(axis='both', labelsize=PLOT_FONT_SIZE)
        ax.grid(True, alpha=0.3)
    for empty_ax in axes.flat[len(PE_TYPES):]:
        empty_ax.axis('off')
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(os.path.join(FIG_DIR, 'ads_vs_epsilon.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print("✓ ads_vs_epsilon.png")

    # Plot 2: ADS + Accuracy dual axis
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('ADS Early Warning: Divergence vs Accuracy', fontsize=PLOT_FONT_SIZE, fontweight='bold')
    for idx, pe in enumerate(PE_TYPES):
        ...
        ax1.set_title(format_condition_label(pe), fontsize=PLOT_FONT_SIZE, fontweight='bold', color=PE_COLORS[pe])
        ax1.tick_params(axis='both', labelsize=PLOT_FONT_SIZE)
        ax2.tick_params(axis='both', labelsize=PLOT_FONT_SIZE)
        ax1.grid(True, alpha=0.3)
        ax1.legend(handles=[l1, l2], loc='lower center', bbox_to_anchor=(0.5, 1.02), ncol=2, fontsize=PLOT_FONT_SIZE, frameon=True, borderaxespad=0.0)
    for empty_ax in axes.flat[len(PE_TYPES):]:
        empty_ax.axis('off')
    plt.tight_layout(rect=[0, 0, 1, 0.94])
    plt.savefig(os.path.join(FIG_DIR, 'ads_early_warning_dual.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print("✓ ads_early_warning_dual.png")

    # Plot 3: Per-layer heatmap
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle('Per-Layer ADS Heatmap (mean over seeds)', fontsize=PLOT_FONT_SIZE, fontweight='bold')
    for idx, pe in enumerate(PE_TYPES):
        ax = axes[idx // 2, idx % 2]
        available = get_available_seeds(pe)
        if not available:
        ...
        ax.set_xticklabels([str(e) for e in epsilons], rotation=45, fontsize=PLOT_FONT_SIZE)
        ...
        cbar.set_label('KL div', fontsize=PLOT_FONT_SIZE)
        cbar.ax.tick_params(labelsize=PLOT_FONT_SIZE)
        ax.tick_params(axis='both', labelsize=PLOT_FONT_SIZE)
    for empty_ax in axes.flat[len(PE_TYPES):]:
        empty_ax.axis('off')
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(os.path.join(FIG_DIR, 'ads_per_layer_heatmap.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    print("Attention Divergence Score (ADS) Experiment — CIFAR-100")
