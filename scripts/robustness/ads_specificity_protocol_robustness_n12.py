...
"""ADS Specificity Experiment — ImageNet-100
=========================================
...
surfaces in ViT-Base models trained on ImageNet-100:
...
each model. It uses a fixed 256-image ADS reference subset. By default,
the reference indices are stored next to the output JSON as
ads_ref_indices_imagenet100.json. Use --ref_indices_path to share the
same reference set with the main ImageNet-100 ADS experiment.

Usage in Colab:
    python /content/ads_specificity_n6.py \
        --models_dir "/content/drive/MyDrive/pe_experiment/results" \
        --val_dir "/content/imagenet100_resized/val" \
        --output_path "/content/drive/MyDrive/ads_tfs_n6/data_n6/ads_specificity.json"

Output:
    - ads_specificity.json
    - ads_ref_indices_imagenet100.json, if not already present
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ADS specificity experiment (ImageNet-100, n=6 default). See module docstring for details.",
...
if __name__ == '__main__':
    print("ADS Specificity Experiment — ImageNet-100")
